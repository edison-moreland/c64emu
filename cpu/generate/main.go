...
func main() {
	...
	f.Const().DefsFunc(func(g *jen.Group) {
		for _, flag := range cpu.Flags {
			...
			flagIdentifiersByBit[flag.Bit] = flagIdentifier
	...
	// decode instruction
	f.Var().Id("ErrInstructionNotFound").Op("=").Qual("errors", "New").Call(jen.Lit("Instruction not found"))
	f.Func().Id("Decode").Params(jen.Id("opcode").Byte()).Params(jen.Id(instructionType), jen.Error()).Block(
		...
		jen.Return(jen.Id(instructionType).Values(), jen.Id("ErrInstructionNotFound")),
	)
